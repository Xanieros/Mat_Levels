...
public class Hooks : IDisposable
{
    ...
    public unsafe Hooks(Plugin plugin)
    {
        ...
        Service.AddonLifecycle.RegisterListener(AddonEvent.PreRequestedUpdate, "ItemDetail", (_, args) => ItemLevelTooltip.RestoreToNormal((AtkUnitBase*)args.Addon.Address));
        Service.AddonLifecycle.RegisterListener(AddonEvent.PostRequestedUpdate, "ItemDetail", (_, args) => plugin.ItemLevelTooltip.OnItemTooltip((AtkUnitBase*)args.Addon.Address));
    }
    ...
    private unsafe byte AgentItemDetailOnItemHoveredDetour(void* a1, void* a2, void* a3, void* a4, uint a5, uint a6, int* a7)
    {
        ...
        try
        {
            plugin.ItemLevelTooltip.LastItemQuantity = a7[5];
        }
        catch (Exception e)
        {
            plugin.ItemLevelTooltip.LastItemQuantity = null;
            Service.Log.Error(e, "Failed to read last item quantity");
        }
